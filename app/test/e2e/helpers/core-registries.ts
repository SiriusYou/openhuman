...
export async function waitForCoreRegistriesPage(timeout = 20_000): Promise<void> {
  await browser.waitUntil(
      ...
      }),
    {
      timeout,
      interval: 250,
      timeoutMsg: 'Core Registries page did not finish loading',
    }
  );
}
...
export async function openRegistryTab(tab: TabName): Promise<void> {
  const opened = await browser.execute((target: string) => {
    const button = Array.from(document.querySelectorAll('[role="tab"]')).find(candidate =>
  ...
  await browser.waitUntil(
      ...
      }, tab),
    {
      timeout: 10_000,
      interval: 250,
      timeoutMsg: `${tab} tab did not become active`,
    }
  );
}
...
export async function clickCollectionRow(
  ...
  await browser.waitUntil(
    async () =>
      browser.execute(
        ({ targetCollection, targetRow }) => {
          const section = Array.from(document.querySelectorAll('section')).find(candidate =>
            candidate.querySelector('h2')?.textContent?.trim() === targetCollection
          );
          if (!section) return false;
          const button = Array.from(section.querySelectorAll('button')).find(candidate =>
            (candidate.textContent ?? '').includes(targetRow)
          ) as HTMLButtonElement | undefined;
          button?.click();
          return Boolean(button);
        },
        { targetCollection: collectionTitle, targetRow: rowText }
      ),
    {
      timeout,
      interval: 250,
...
export async function clickActionButton(label: string, timeout = 10_000): Promise<void> {
  await browser.waitUntil(
      ...
      }, label),
    {
      timeout,
      interval: 250,
      timeoutMsg: `Button "${label}" was not clickable`,
    }
  );
}
...
export async function waitForDetailHeading(title: string, timeout = 20_000): Promise<void> {
  await browser.waitUntil(
      ...
      }, title),
    {
      timeout,
      interval: 250,
      timeoutMsg: `Detail heading "${title}" did not appear`,
    }
  );
}
...
export async function waitForText(text: string, timeout = 20_000): Promise<void> {
  await browser.waitUntil(
    async () =>
      browser.execute((target: string) => document.body.textContent?.includes(target) === true, text),
    {
      timeout,
      interval: 250,
      timeoutMsg: `Text "${text}" did not appear`,
    }
  );
}
